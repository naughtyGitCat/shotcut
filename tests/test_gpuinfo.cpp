...
private slots:
    ...
    void prefersVendorFamily()
    {
        ...
        QCOMPARE(preferredHardwareVcodec(hw, "libx264", kGpuVendorNvidia), QString("h264_nvenc"));
        QCOMPARE(preferredHardwareVcodec(hw, "libx265", kGpuVendorNvidia), QString("hevc_nvenc"));
        QCOMPARE(preferredHardwareVcodec(hw, "libx264", kGpuVendorAmd), QString("h264_amf"));
        QCOMPARE(preferredHardwareVcodec(hw, "libx265", kGpuVendorAmd), QString("hevc_amf"));
    }

    void prefersQsvForIntel()
    {
        const QStringList hw{"h264_nvenc", "h264_qsv"};
        QCOMPARE(preferredHardwareVcodec(hw, "libx264", kGpuVendorIntel), QString("h264_qsv"));
    }
    ...
    void fallsBackToFirstWhenFamilyMissing()
    {
        ...
        QCOMPARE(preferredHardwareVcodec(hw, "libx264", kGpuVendorNvidia), QString("h264_amf"));
    }
    ...
    void fallsBackToFirstWhenAutomatic()
    {
        ...
        const QStringList hw{"h264_amf", "h264_nvenc"};
        QCOMPARE(preferredHardwareVcodec(hw, "libx264", 0u), QString("h264_amf"));
    }

    void matchesCodecType()
    {
        const QStringList hw{"h264_nvenc", "hevc_nvenc", "av1_nvenc", "vp9_qsv"};
        QCOMPARE(preferredHardwareVcodec(hw, "libsvtav1", kGpuVendorNvidia), QString("av1_nvenc"));
        // No vp9 NVENC exists -> fall back to the first vp9 encoder.
        QCOMPARE(preferredHardwareVcodec(hw, "libvpx-vp9", kGpuVendorNvidia), QString("vp9_qsv"));
    }

    void returnsEmptyWhenNoneCompatible()
    {
        const QStringList hw{"h264_nvenc", "hevc_nvenc"};
        QVERIFY(preferredHardwareVcodec(hw, "libvpx-vp9", kGpuVendorNvidia).isEmpty());
        QVERIFY(preferredHardwareVcodec(QStringList(), "libx264", kGpuVendorNvidia).isEmpty());
    }
